...
class Node:
    ...
    def get_heuristic(self):
        """checking for winner, if there is one, returns infinity or -infinity"""
        win = self.get_winner()
        if win == 1:
            return float("inf")
        if win == -1:
            return -float("inf")
        """if there is no winner returns heuristic based on table"""
    ...
    while True:
        ...
        if user_turn:
            ...
            for child in cur_node.children:
                val_minimax = minimax(child, depth, True)
                if val_minimax < min_p:
                    ...
                    temp.append(child)
                next_node = temp[randint(0, len(temp)-1)]
            cur_node = next_node
            user_turn = True
            print_board(cur_node.board, user_start)
